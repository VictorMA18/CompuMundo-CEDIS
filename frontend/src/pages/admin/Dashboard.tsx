...
export default function Dashboard() {
  ...
  const lineChartData = {
    ...
    datasets: [{
      ...
      tension: 0.4, fill: true, pointBackgroundColor: '#fff', pointBorderColor: '#B31818'
    }]
  };
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '25px' }}>
        <div className="chart-container" style={{ gridColumn: 'span 8', backgroundColor:'white', padding:'20px', borderRadius:'8px', border:'1px solid #eee' }}>
          <h4 style={{margin:'0 0 15px 0', color:'#444'}}>Evolución Histórica</h4>
          <div style={{ height: '300px' }}>
            <Line data={lineChartData} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { display: false } } } }} />
          </div>
        </div>

        <div className="chart-container" style={{ gridColumn: 'span 4', backgroundColor:'white', padding:'20px', borderRadius:'8px', border:'1px solid #eee' }}>
        ...
        <div className="chart-container" style={{ gridColumn: 'span 12', backgroundColor:'white', padding:'20px', borderRadius:'8px', border:'1px solid #eee' }}>
          <h4 style={{margin:'0 0 15px 0', color:'#444'}}>Documentos Más Solicitados (Color por Categoría)</h4>
          <div style={{ height: '250px' }}>
            <Bar data={barChartData} options={{ indexAxis: 'y', maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { display: false } } } }} />
          </div>
        </div>
      </div>
    </div>
...
function RedCard({ title, value, icon }: any) {
  return (
    <div style={{ backgroundColor: '#B31818', borderRadius: '12px', padding: '25px', color: 'white', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '140px', boxShadow: '0 10px 20px rgba(179, 24, 24, 0.2)', position: 'relative', overflow: 'hidden' }}>
