...
const CHART_COLORS = [
  "#B31818", "#2D3748", "#E53E3E", "#718096", 
  "#F6AD55", "#4A5568", "#FC8181", "#CBD5E0"
];
...
export default function Dashboard() {
  ...
  const [graphData, setGraphData] = useState({
    ...
    topDocs: { labels: [] as string[], data: [] as number[], colors: [] as string[] },
  });
    ...
    try {
      ...
      prestamos.forEach((p: any) => {
        p.detalles.forEach((d: any) => {
          ...
          const catNombre = d.materialBibliografico?.categoria?.CatNom || "General";
          if (!docMap[titulo]) docMap[titulo] = { count: 0, category: catNombre };
          docMap[titulo].count += 1;
        });
      });

      const top5 = Object.entries(docMap).sort(([, a], [, b]) => b.count - a.count).slice(0, 5);
      const barColors = top5.map(([, info]) => newColorMap[info.category] || "#ccc");
      ...
      setGraphData({
        ...
        topDocs: {
          labels: top5.map(([k]) => k.length > 20 ? k.substring(0, 20) + "..." : k),
  ...
  if (loading) return <div className="loading-container">Cargando estadísticas...</div>;

  return (
    <div className="dashboard-wrapper" style={{ padding: "24px", backgroundColor: "#f8f9fa" }}>
      ...
      <div style={{ 
        ...
        <StatCard title="Total Préstamos" value={kpis.totalPrestamos} icon="📊" color="#B31818" />
        <StatCard title="Catálogo Actual" value={kpis.totalDocs} icon="📚" color="#2D3748" />
        <StatCard title="Usuarios Activos" value={kpis.totalLectores} icon="👤" color="#B31818" />
        <StatCard title="Tasa Morosidad" value={`${kpis.tasaMorosidad}%`} icon="⚠️" color="#E53E3E" />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(12, 1fr)", gap: "24px" }}>
          ...
          <Doughnut 
            data={{
              ...
              datasets: [{
                ...
                backgroundColor: graphData.categorias.labels.map(c => categoryColorMap[c]),
                hoverOffset: 10
              }]
            }}
            options={{
        ...
        <ChartBox title="Documentos más solicitados (por categoría)" span="12">
          <Bar 
            data={{
...
function StatCard({ title, value, icon, color }: any) {
  return (
    <div style={{
      ...
      <div>
        <p style={{ margin: 0, fontSize: "0.875rem", color: "#718096", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.025em" }}>{title}</p>
        <h3 style={{ margin: "4px 0 0 0", fontSize: "1.875rem", fontWeight: 700, color: "#1A202C" }}>{value}</h3>
      </div>
      <div style={{ fontSize: "2rem", backgroundColor: "#F7FAFC", padding: "12px", borderRadius: "12px" }}>{icon}</div>
    </div>
  );
}
...
function ChartBox({ title, children, span }: any) {
  return (
    <div style={{
      ...
      <h4 style={{ margin: "0 0 20px 0", color: "#2D3748", fontSize: "1.1rem", fontWeight: 600 }}>{title}</h4>
...
const chartOptions = {
  ...
  plugins: { legend: { display: false } },
  scales: {
    y: { beginAtZero: true, grid: { color: "#EDF2F7" }, border: { display: false } },
